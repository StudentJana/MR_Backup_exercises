    ...
    : Node("ekf", options)
{
    ...
    // load map from file
    if (param_->map_linesegments_file != "")
    {
        cv::FileStorage fs(param_->map_linesegments_file, cv::FileStorage::READ);
        cv::Mat_<double> l;
        fs["line segments"] >> l;
        std::vector<tuw::LineSegment2D> line_vect;
        for (int i = 0; i < l.rows; i++)
            line_vect.push_back(tuw::LineSegment2D(l(i, 0), l(i, 1), l(i, 2), l(i, 3)));
        filter_->set_map(tuw::LineSegments2D(line_vect));
    }
...
void EKFNode::callback_laser(const LaserScanMsg::SharedPtr msg)
{
    ...
    pose_sensor.set(t.transform.translation.x, t.transform.translation.y, QuaternionToYaw(t.transform.rotation));
