...
namespace mr
{
    ...
    public:
        ...
        enum Level : unsigned int
        {
            ...
            COMPUTE_BEAMS_USED = 3,
            PLOT_Z_EXPECTED = 3,
            COMPUTE_LIKELIHOOD_FIELD = 4,
            COMPUTE_Z_EXPECTED = 4,
            PLOT_SAMPLES = 5,
            COMPUTE_PSEUDO_DENSITY_LOOKUP = 5,
        ...
        enum sensor_model : unsigned int
        {
            MODEL_NA = 0,
            MODEL_BEAM = 1,
            MODEL_SCAN = 2,
        ...
        sensor_model sensor_model;                             /// sensor model to use
        double sigma_hit;                                      /// sensor model parameters
        double lambada_short;                                  /// sensor model parameters
        double z_hit;                                          /// sensor model parameters
        double z_short;                                        /// sensor model parameters
        double z_rand;                                         /// sensor model parameters
        double z_max;                                          /// sensor model parameters
        int pseudo_density_scale;                              /// sensor model pixel/meter
        bool draw_z_exp;                                       /// draw the expected measurment
        double gnuplot_z_exp;                                  /// sensor defines where to plot the pseudo-denisty for debuging
