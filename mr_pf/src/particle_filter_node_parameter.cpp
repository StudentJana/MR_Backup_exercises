...
void ParticleFilterNodeParameter::declare_parameters()
{
    ...
    declare_default_parameter<std::string>("sensor_model", "beam", "sensor model used: beam or scan");
    declare_default_parameter<double>("sensor_model.sigma_hit", 0.5, 0.001, 1., 0.001, "Standard distribution for likelihood field");
    declare_default_parameter<double>("sensor_model.lambada_short", 0.5, 0., 1., 0.001, "lambada_short");
    declare_default_parameter<int>("sensor_model.pseudo_density_scale", 100, 1, 1000, 1, "scale used for the lookup on the pseudo-density fnc [pixel/meter]");
    declare_default_parameter<double>("sensor_model.z_hit", 0.7, 0., 1., 0.001, "z hit");
    declare_default_parameter<double>("sensor_model.z_short", 0.4, 0., 1., 0.001, "z short");
    declare_default_parameter<double>("sensor_model.z_rand", 0.4, 0., 1., 0.001, "z rand");
    declare_default_parameter<double>("sensor_model.z_max", 0.001, 0., 1., 0.001, "z max");
    declare_default_parameter<double>("sensor_model.rate_of_beams_used", 0.2, 0., 1., 0.001, "rate of beams used for weighting");
    declare_default_parameter<double>("sensor_model.gnuplot_z_exp", 3.0, 0., 10, 0.1, "Where to plut the pseudo density function for debugging");
...
void ParticleFilterNodeParameter::callback_update_parameters()
{
    ...
    n_.get_parameter<double>("sensor_model.lambada_short", lambada_short);
    n_.get_parameter<int>("sensor_model.pseudo_density_scale", pseudo_density_scale);
    ...
    n_.get_parameter<double>("sensor_model.gnuplot_z_exp", gnuplot_z_exp);
    n_.get_parameter<bool>("sensor_model.draw_z_exp", draw_z_exp);
    ...
    sensor_model_str = n_.get_parameter("sensor_model").get_parameter_value().get<std::string>();
    ...
    if (sensor_model_str.compare("beam") == 0)
        sensor_model = MODEL_BEAM;
    else if (sensor_model_str.compare("scan") == 0)
        sensor_model = MODEL_SCAN;
    else
    {
        sensor_model = MODEL_SCAN;
        RCLCPP_FATAL(n_.get_logger(), "The sensor model \'%s\' is not supported: use \'beam\' or \'scan\'", sensor_model_str.c_str());
    }
    /**
     * @ToDo Sensor Model
